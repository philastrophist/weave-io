...
from .readquery.writing.statements import MergeNode
...
class Graphable(metaclass=GraphableMeta):
    ...
    def __init__(self, predecessors, successors=None):
        if successors is None:
            successors = {}
        G = QueryGraph.get_context()  # type: QueryGraph
        merge_strategy = self.__class__.merge_strategy()
        if  merge_strategy == 'NODE FIRST':
            shared_parent = self.get_shared_parent()
            statement = MergeNode(self.neotypes, self.neoidentproperties, self.neoproperties, G.on_collision, G, parameters)
            node = G.add_merge(statement)
            ObjectQuery._spawn(shared_parent, node, obj=self.__class__.__name__, single=True)
