...
def _template_aggregator(string_op, predicate, python_func: Callable, item: BaseQuery, wrt: BaseQuery = None,
                         ...
                         op_name=None, distinct=False, args=None, kwargs=None):
    ...
    if op_name is None:
        op_name = string_op
    try:
        return item._aggregate(wrt, string_op, op_name, predicate, expected_dtype, returns_dtype, remove_infs, distinct)
...
def array(item, wrt=None, distinct=False, *args, **kwargs):
    return _template_aggregator('collect', False, list, item, wrt, distinct=distinct, args=args, kwargs=kwargs)