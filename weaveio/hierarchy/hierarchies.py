...
class IndexableHierarchy(BaseHierarchy):
	def _index_by_address(self, address):
		...
		hierarchy_matches = []
		for hierarchy_type in self.hierarchy_types:
...
class HomogeneousHierarchy(IndexableHierarchy):
	...
	def __getitem__(self, key):
		if isinstance(key, int):
			self.instantiate_hierarchies()
			return self.hierarchies[key]
		elif isinstance(key, str):
			self.address.refine(**{self.hierarchy_type.key: key})
		elif isinstance(key, Address):
			return HomogeneousHierarchy(self.base, self.address.combine(address), self.hierarchy_type)
		else:
			raise TypeError(f"{key} must be a int/str/Address")
...
class Hierarchy(BaseHierarchy):
	...
	def __init__(self, base, address):
		super().__init__(base, address)
		self._requires = {i.name: i for i in self.requires}
		self._required_by = {i.name: i for i in self.required_by}
		self._produces = {i.name: i for i in self.produces}
	...
	def _subhierarchy_list(self, hierarchy_type_name):
		...
		Take the address and combine it
		"""
		return HomogeneousHierarchy(self.base, address, self._required_by[hierarchy_type_name])
